describe('checkEffortsPageContent', () => {
      it('user should be able to see valid and grammatically correct information', () => {
        ...
        cy.get('#routeInfos').contains('Link: https://ultrabalaton.hu/nnultrabalaton/')

        cy.get('#hikingDiv').contains('Túrázva')
        cy.get('#hikingDiv').contains('Még senki sem teljesítette ezt az útvonalat ilyen módon. Legyél te az első!')

        cy.get('#runningDiv').contains('Futva')
        cy.get('#runningDiv').contains('Még senki sem teljesítette ezt az útvonalat ilyen módon. Legyél te az első!')

        cy.get('#bikeDiv').contains('Kerékpározva')
        cy.get('#bikeDiv').contains('Még senki sem teljesítette ezt az útvonalat ilyen módon. Legyél te az első!')
